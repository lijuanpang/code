xu = 5;
xl = -5;
L = randint(1,1,[5,11])%禁忌长度取[5,11]之间的随机数，去矩阵（1,1）范围在【5,11】内的数字
Ca = 5;%邻域解个数
G = 200;%仅计算法的最大迭代次数
w = 1;%自适应权重系数
tabu = [];%禁忌表
x0=rand(1,2)*(xu-xl)+xl;%随机产生初始解，rand(1,2)是1以内的一行两列的矩阵
...
while g <G
    ...
    if delta1 <= 0 
        ...
        if delta2 > 0 %候选解比目前最优解优
            ...
            r = 0;
            for m = 1:M
                if tabu(m,1)==candidate(g).key(1) & tabu(m,2)==candidate(g).key(2)%%%%%%%%%
                    r = 1;
                end
            end
            if r == 0
